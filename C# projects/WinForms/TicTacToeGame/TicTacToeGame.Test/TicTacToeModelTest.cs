...
namespace ELTE.TicTacToeGame.Test
{
    ...
    public class TicTacToeModelTest // egységteszt osztály
    {
        ...
        public void Initialize() // teszt inicializálása
        {
            ...
            _mock.Setup(mock => mock.LoadAsync(It.IsAny<String>())).Returns(Task.Run(() => Enumerable.Repeat(Player.NoPlayer, 9).ToArray()));
            // a mock a LoadAsync műveletben bármilyen paraméterre egy üres táblának a tömbjét fogja visszaadni
        ...
        [TestMethod]
        public async Task TicTacToeGameLoadAsyncTest()
        {
        ...
        [TestMethod]
        public async Task TicTacToeGameSaveAsyncTest()
        {
            ...
            _mock.Verify(mock => mock.SaveAsync(String.Empty, It.IsAny<Player[]>()), Times.Once());
        }
    }
}
